...
function render_kick_trajectory(ts, zs, qs, walker, steps, fps, body, physics,   
    ...
    fig = Figure()
    ax = Axis(fig[1,1], aspect=DataAspect(), limits=(-2,4,-0.1,2)) 
    lines!(ax, [-2,4], [-0.015, -0.015], color=:black, linewidth=10) 
    ...
    Makie.scatter!(ax, hinge; marker=:rect,  linewidth=4, strokecolor=:black, color=:black, markersize= 50)
    Makie.scatter!(ax, center; marker=:circle,  linewidth=4, strokecolor=:blue, markersize= 50)
    lines!(ax, stance_foot, color=:red, linewidth=6)
    lines!(ax, swing_foot, color=:blue, linewidth=6)
    lines!(ax, stance_feet, color=:red, linewidth=6)
    lines!(ax, swing_feet, color=:blue, linewidth=6)
    ...
    function ballanimstep!(t) 
        if t <= N
            t = Int(round(t))
            H = [[cos(qs[3, t]) -sin(qs[3, t]) qs[1, t]];
                    [sin(qs[3, t]) cos(qs[3,t]) qs[2, t]];
                    [0. 0. 1]]
            wc = H * lc
            center[] = Point2f0(wc[1], wc[2])
